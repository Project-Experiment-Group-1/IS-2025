...
EXP_AVG = 0.5  # 70% the new value 30% the old value
...
ls_baseline_arousal = []
ls_baseline_valence = []
CALIBRATION_FRAMES = 100  # 我们将使用 100 帧来进行校准 (大约 3-5 秒)
baseline_valence = 0.0  # 最终计算出的“愉悦度偏移量”
baseline_arousal = 0.0  # 最终计算出的“唤醒度偏移量”
...
if __name__ == "__main__":
    ...
    # --- 校准阶段 ---
    print(f"--- 开始校准 ---")
    print(f"请保持【面无表情】(中性表情)，持续 {CALIBRATION_FRAMES} 帧...")
    ...
    for i in range(CALIBRATION_FRAMES):
        ...
        # 在画面上显示提示
        cv2.putText(frame, "CALIBRATING: Hold NEUTRAL face", (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)  # 红色提示
        ...
        if len(faces) > 0:
            ...
            # 持续收集“中性”时的 VA 值
            ls_baseline_valence.append(dict_emotions['emotions']['valence'])
            ls_baseline_arousal.append(dict_emotions['emotions']['arousal'])
    ...
    # --- 计算基准线 ---
    if not ls_baseline_valence:  # 如果在 100 帧里一次都没检测到脸
        print("校准失败：未检测到人脸。将使用 0.0 作为基准。")
    else:
        # 计算这 100 帧的“平均偏移量”
        baseline_valence = np.mean(ls_baseline_valence)
        baseline_arousal = np.mean(ls_baseline_arousal)

    print(f"--- 校准完成 ---")
    print(f"您的愉悦度基准线 (Valence Offset): {baseline_valence:.2f}")
    print(f"您的唤醒度基准线 (Arousal Offset): {baseline_arousal:.2f}")
    print("--- 开始实时检测 ---")
    ...
    while True:  # for 300 frames
        ...
        if len(faces) > 0 and f % 20 == 0:  # if there are faces detected
            ...
            valence = valence - baseline_valence
            arousal = arousal - baseline_arousal
            ...
            print(
                f"Valence: {disp_valence:.2f} | "
                f"Arousal: {disp_arousal:.2f} | "
            ...
            if landmarks is not None:
                ...
                plt.pause(0.01)  # needed for updating the graph
                plt.show()
